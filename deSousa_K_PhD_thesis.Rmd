...
During the implementation of the activities in this project we developed a series of tools and methods to accommodate the *tricot* data into a dynamic workflow in R [@RCoreTeam] that could update the models and provide new insights as the database grows in number of datapoints (Fig. 10). This were our first steps in moving the *tricot* data analysis into a machine learning framework [@James2013]. The workflow follows the steps: (*A*) Several participants contribute with small tasks, as explained in the *tricot* description, (*B*) Explanatory variables are added (*e.g.* using geographical coordinates and planting dates, or even DNA markers), (*C*) Model selection to find the variables that best explain the data (*D*) Automated reports can be generated and provide feedback to participants, (*E*) A stable Plackett-Luce tree is used for further analysis and inference. 

The *tricot* data is storaged in the cloud under the ClimMob platform (https://climmob.net/blog/). We developed `ClimMobTools` [@climmobtools] an API client that allows the user to fetch their *tricot* data into the R section preserving the original data.

We focused on generalisation of the tools, which means that they can be used in other domains outside the the proposed workflow. First we moved the data analysis from Bradley-Terry model to Plackett-Luce, since, unlike the former, it considerer the entire permutation instead of pairwise comparisons, allowing for fast computation and higher accuracy (Fig. 10A). The R package `PlackettLuce` [@Turner2020] implements this tool and is a key element in the workflow.

```{r workflow, fig.cap = "Workflow to analyse crowdsourced citizen science data. Several participants contribute with small tasks, all data is combined using rankings (\\textbf{A}). Covariates (climatic or DNA markers) are linked to the rankings using georreferenced information and sowing dates (\\textbf{B}). Selection of the most relevant covariate(s) using forward selection aproach (\\textbf{C}). Automated reports can be generated to give feedback to participants in A (\\textbf{D}). A stable tree is used for further analysis and interpretation (\\textbf{E}).", echo=FALSE, out.width = '90%', fig.align="center"}
knitr::include_graphics("display-items/workflow.png")
```

To link environmental covariates to the rankings (Fig. 10B) we developed the R packages `climatrends` [@climatrends] and `chirps` [@chirps]. The package `climatrends` supports analysis of trends in climate change, ecological and crop modelling by computing environmental variables with temperature and precipitation data. It started as a set of R scripts used in Paper I [@deSousa2018] and, as it grew in complexity and demand, it was implemented as a R package. The tool not only supports the proposed workflow but can be used in ecology and climatology, as described in its application paper. The package `chirps` offers an application programming interface (API) client for the CHIRPS data [@Funk2015] in R. With the package, users can fetch CHIRPS data into the R section and compute rainfall indices that can be linked to the workflow or used in other domains. 

For step C (Fig. 10C) we developed the R package `gosset` [@gosset] which provides the methods to analyse the *tricot* and other metadata, from collating data (transforming into rankings) to model selection and visualisation. The cross-validation, forward selection, pseudo-$R^2$, and Kendal $\tau$ computation used in Paper III and Paper IV are implemented in this package. 
