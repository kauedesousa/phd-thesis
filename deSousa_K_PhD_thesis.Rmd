...
Agriculture is a dynamic activity that constantly evolve since its origin. It sustains food and other goods that the global population depends on. Aiming to ensure global food security, the sector has evolved dramatically, especially over the last century with the introduction of high-yielding crops, improved technology, and pathogen resistant and nutrition enriched varieties, to name a few. Still, according to the Food and Agriculture Organisation of the United Nations (FAO), the food security issue is still present. In 2019, around 26% of the world population experienced either a moderate or severe level of food insecurity. Climate change makes the challenge of food security even more accurate. To cope with climate change and global food insecurity, the whole food system needs to be transformed. It is argued that increased agrobiodiversity through farm diversification and varietal selection can help farmers to cope with the negative effects of climate change while ensuring food security. However, such approaches have been difficult to scale up. One could argue that we often lack information to understand the contexts that drives farmers' adaptation decisions and how to develop recommendations domains for adaptation. In this project I developed methods and tools to support farmers and stakeholders in adapting to a changing climate. I compiled evidence and insights from three continents to improve the understanding of the food systems at the farm level, and specifically for smallholders. I provide evidence for the different levels of biological organisation: species, focusing on trees as slow grower organisms for interspecific diversification; varieties, looking for locally adapted phenotypes; and genotypes (genes) focusing on G $\times$ E interactions to support crop breeding for intraspecific diversification. The results from one research site showed that farmers have a clear preference to a set of adaptation strategies (Paper 1), with reforestation (agroforestry) as the first choice disregarding socio-economic characteristics. Crop variety management is the least preferred choice of the top-5. I assessed how suitable to climate change are the habitats of the most common trees species used in agroforestry (Paper 2). The results showed that the most preferred trees are the most vulnerable, and requires a re-design of the current agroforestry systems using under-utilised species that are already present in the current systems in low densities. On the other hand, I employed a citizen science approach that could scale agricultural experimentation and help farmers in select the right crop variety for their farms (Paper 3). The results showed that the approach reduce geographic sampling bias and can be scaled across space and time to provide tailored recommendations on crop varieties. I also show that linking the farmer-generated data to scientist-generated data (*e.g.* DNA, atmospheric data) can support breeding programs targeting challenging crop environments using a data-driven decentralised approach (Paper 4). The approach is fully replicable, and part of its workflow is presented in this thesis (Paper 5 and Paper 6). Overall, the results of this study are a starting point to develop lines of research that support the development of recommendation domains to adapt agricultural systems to a changing climate.

\pagebreak

# Sammendrag

Landbruk er en dynamisk aktivitet som stadig utvikler seg siden opprinnelsen. Den opprettholder mat og andre varer som verdens befolkning er avhengig av. Siktet for å sikre global matsikkerhet, har sektoren utviklet seg dramatisk, spesielt i løpet av det siste århundret med innføring av avlinger med høy avling, forbedret teknologi og patogenresistente og ernæringsberikede varianter, for å nevne noen. I følge FNs mat- og jordbruksorganisasjon (FAO) er matsikkerhetsspørsmålet fortsatt til stede. I 2019 opplevde rundt 26% av verdens befolkning enten et moderat eller alvorlig nivå av matusikkerhet. Klimaendringer gjør utfordringen med matsikkerhet enda mer nøyaktig. For å takle klimaendringer og global matusikkerhet, må hele matvaresystemet transformeres. Det hevdes at økt biologisk mangfold gjennom diversifisering av gård og sortvalg kan hjelpe bønder til å takle de negative effektene av klimaendringene, samtidig som det sikrer matsikkerhet. Imidlertid har slike tilnærminger vært vanskelig å oppskalere. Man kan hevde at vi ofte mangler informasjon for å forstå sammenhenger som driver bøndenes tilpasningsbeslutninger og hvordan man kan utvikle anbefalinger domener for tilpasning. I dette prosjektet utviklet jeg metoder og verktøy for å støtte bønder og interessenter i å tilpasse seg et skiftende klima. Jeg samlet bevis og innsikt fra tre kontinenter for å forbedre forståelsen av matvaresystemene på gårdsnivå, og spesielt for småbrukere. Jeg gir bevis for de ulike nivåene av biologisk organisasjon: arter, med fokus på trær som langsomt dyrkerorganismer for interspesifikk diversifisering; varianter, på jakt etter lokalt tilpassede fenotyper; og genotyper (gener) med fokus på G $\times$ E-interaksjoner for å støtte avlsavl for intraspesifikk diversifisering. Resultatene fra ett forskningssted viste at bønder har en klar preferanse for et sett med tilpasningsstrategier (Paper 1), med skogplanting (agroforestry) som førstevalg uten å ta hensyn til sosioøkonomiske egenskaper. Avlsforvaltning er det minst foretrukne valget av topp-5. Jeg vurderte hvor egnet klimaendringene er habitatene til de vanligste treslagene som brukes i skogbruket (Paper 2). Resultatene viste at de mest foretrukne trærne er de mest sårbare, og krever en ny utforming av dagens skogbrukssystemer ved bruk av underutnyttede arter som allerede er tilstede i dagens systemer i lave tettheter. På den annen side brukte jeg en innbyggervitenskapelig tilnærming som kunne skalere landbrukseksperimentering og hjelpe bønder med å velge riktig avlingsvariant for gårdene sine (Paper 3). Resultatene viste at tilnærmingen reduserer geografisk prøvetakingsforstyrrelse og kan skaleres over tid og tid for å gi skreddersydde anbefalinger om avlingsvarianter. Jeg viser også at å knytte bonde-genererte data til forsker-genererte data (* f.eks. * DNA, atmosfæriske data) kan støtte avlsprogrammer rettet mot utfordrende avlingsmiljøer ved hjelp av en datadrevet desentralisert tilnærming (Paper 4). Tilnærmingen er fullt replikerbar, og en del av arbeidsflyten presenteres i denne oppgaven (Paper 5 og Paper 6). Samlet sett er resultatene av denne studien et utgangspunkt for å utvikle forskningslinjer som støtter utviklingen av anbefalingsdomener for å tilpasse landbrukssystemer til et klima i endring.

\pagebreak

# Preface

This thesis summarises an amazing chapter of my personal and professional life. Growing up in the middle of the Brazilian Amazon, I always had interest for plants, first to climb then but later to actually study and research how they function and grow. My grandmother was my sponsor, supporting my interest to cultivate and harvest beans in our garden (thanks gradma!). With my parents I learned that the way people interact with the environment is also an important aspect to look at. As I grew up, I met scientists that helped me to put these lessons into a more organised and scientific way. First at UFAM (my *Alma mater*) and INPA where I realised that, yes, I want to be a scientist. For this, thank you all. At CATIE, I learned how to integrate all the complexity of landscapes into coherent approaches, including people, trees and crops. Lessons that were taken to a higher level when I joined ICRAF, Bioversity International and INN University. During this time, I learned from a amazing group of scientist that gave me all the inputs that I used to develop this thesis. It summarises my interests in plants and their interplay with people, things that, as I said, I developed since I was a child. That is why this thesis looks for the different components of a landscape, first the people, then the trees and then the crops from where they take most of their food. I really enjoyed to write this thesis. Thanks to the unbelievable opportunities that I had and all the nice and brilliant people that I met during this path. I wish that the children growing in poor regions in the tropics have the same opportunities that I had. 
...
**1. Can climate awareness influence sustainable adaptation decisions on smallholder farms? (Paper 1)**
...
We estimated the *worth* parameters (relative importance) using the maximum likelihood estimation with the Bradley-Terry (BT) model [@Bradley1952]. In the BT model, each $m$ individuals choice is compared with one another in pairs and the number of times $i$ is preferred over $j$ is computed. The outcomes of different pairings are assumed to be independent, and the log-likelihood based on the BT model is:
