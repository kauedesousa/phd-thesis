...
These issues highlight the need of rapid, tailored and straightforward solutions to help farmers to deal with uncertain climates. In this thesis I analysed diversification approaches to bring new insights to generate accurate climate-friendly recommendations for farms among different climatic spaces. I focussed on smallholder farming, which were identified as the most vulnerable to climate change [@Lipper2014; @Harvey2014; @Vermeulen2012] being conducted in more than 570 million farms worldwide [@Lowder2016], contribute to around 30-34% of global food supply [@Ricciardi2018], and are central for the conservation of agrobiodiversity [@Altieri2017].

# Objectives

In this thesis I focussed in providing insights and develop methods to support farmers and stakeholders in adapting agricultural systems to a changing climate, specifically for smallholders, by answering the following questions:
...
The relative importance of the different strategies was measured using the Bradley-Terry model (BTM) [@Bradley1952], which estimates the *worth* parameter (relative importance) in pairwise comparisons which can be described using the following equation. 
...
For the analysis of the ranking data generated by farmers, we used the Plackett–Luce model (PLM) [@Luce1959; @Plackett1975], implemented in R with the package `PlackettLuce` [@Turner2020]. Alike BTM, explained in the previous section, the Plackett-Luce model estimates the *worth* parameter, but in that case it is estimated as the probability that one item (a variety) wins against all the others in the permutation (BTM measures as pairwise comparison). The model determines the values of positive-valued parameters $\alpha_i$ (*worth*) associated with each variety $i$. These parameters $\alpha$ are related to the probability ($P$) that variety $i$ wins against all other $n$ varieties in the following way:
...
\Huge \flushright \textbf{\colorbox{gray!60}{Paper VI}}
\
