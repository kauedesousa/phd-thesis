...
The ClimMob platform (https://climmob.net/blog/) storages the *tricot* data in the cloud. To start the workflow, we developed `ClimMobTools` [@climmobtools] an application programming interface (API) client that allows the user to fetch their *tricot* data into the R section preserving the original data. This package is designed to work with the ClimMob platform and has no clear application in other domains. 

For the other tools, we focused on generalisation, which means that they can be used in other domains outside the the proposed workflow. First, we moved the data analysis from Bradley-Terry model to Plackett-Luce, since, unlike the former, it considerer the entire permutation instead of pairwise comparisons, allowing for fast computation and higher accuracy (Fig. 10A). The R package `PlackettLuce` [@Turner2020] implements this tool and is a key element in the workflow.

```{r workflow, fig.cap = "Workflow to analyse crowdsourced citizen science data. Several participants contribute with small tasks, all data is combined using rankings (\\textbf{A}). Covariates are linked to the rankings using georreferenced information and sowing dates (\\textbf{B}). Selection of the most relevant covariate(s) using forward selection aproach (\\textbf{C}). Automated reports can be generated to give feedback to participants in A (\\textbf{D}). A stable tree is used for further analysis and interpretation (\\textbf{E}).", echo=FALSE, out.width = '90%', fig.align="center"}
knitr::include_graphics("display-items/workflow.png")
```

To link environmental covariates with the rankings (Fig. 10B) we developed the R packages `climatrends` [@climatrends] and `chirps` [@chirps]. The package `climatrends` supports analysis of trends in climate change, ecological and crop modelling by computing temperature and precipitation indices, the data required to compute such indices can be provided by the user or alternatively using the R package `nasapower` [@Sparks2018] and `chirps`, further details are given in the dynamic documentation in the package's website. The package `climatrends` started as a set of R scripts used in Paper I [@deSousa2018] and, as it grew in complexity and demand, it was implemented as a R package. The package not only supports the proposed workflow but can be used in ecology and climatology. The package `chirps` offers an API client for the CHIRPS data [@Funk2015] in R. With the package, users can fetch CHIRPS data into the R section and compute rainfall indices that can be linked to the workflow or used in other research domains. 

The R package `gosset` [@gosset] which provides the methods to analyse the *tricot* and other metadata, from collating data (transforming into rankings) to model selection and visualisation. It supports steps C, D and E of the workflow. The cross-validation and forward selection procedure, as well as the Akaike weights, pseudo-$R^2$, Kendal $\tau$, and worst regret computation used in Paper III and Paper IV are implemented in this package. Additionally, it also has visualisation tools that help the explanatory analysis of any ranking data.
...
* Use big data to infer patterns in crop $\times$ tree interactions to deliver recommendations based on the portfolio theory [@Blandon1985]
* Potential to use the citizen science data with crop models
* Tricot to assess consumers' preference as a final step in breeding programs 
...
\
```{r paper5, echo=FALSE, out.width = '100%', fig.align="center"}
knitr::include_graphics("display-items/paper5.jpg")
```

\normalsize Tapajós River (an affluent to the Amazon River) in Santarém, Brazil. Credit: K. de Sousa
